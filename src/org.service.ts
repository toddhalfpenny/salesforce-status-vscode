...
export async function getAlias(path: string) {
  try {
    const config = JSON.parse(readFileSync(`${path}/${SF_DIR}${CUR_ORG_FILE}`).toString());
...
export async function createMD(status: any, instanceRec: any): Promise<string> {
  return new Promise(async (resolve, reject) => {
    ...
    const orgEvents:OrgEvent[] = await parseOrgEvents(status as StatusCallResult);
    ...
    for (const event of orgEvents) {
      ...
      const eventUrl = (event.type === "Maintenance") ?
...
function parseOrgEvents(statusResponse:StatusCallResult): Promise<OrgEvent[]> {
  return new Promise(async (resolve, reject) => {
    ...
    const maintenanceEvents: OrgEvent[] = statusResponse.Maintenances.map( inEvent => {
